...
class TestSemanticTypeLine(unittest.TestCase):
    ...
    def test_constructor(self):
        """Test that all SemanticTypeLine objects got built."""
        for semtypeObject in self.semtypeObjects:
            self.assertTrue(isinstance(semtypeObject, rxnorm.SemanticTypeLine))

    def test_semtype_get(self):
        """Test that SemanticTypeLine.semType property yields the expected 
        values from our test objects."""
        i = 0
    ...
    def test_semtype_readonly(self):
        """Test that the .semType property is immutable."""
    ...
    def test_CUI_get(self):
        """Test that .CUI property yields the expected values 
        from our test objects."""
        i = 0
        while i < len(self.semtypeObjects):
    ...
    def test_CUI_readonly(self):
        """Test that the .CUI property is immutable."""
        stl = random.sample(self.semtypeObjects, 1)[0]
        self.assertRaises(AttributeError, stl.__setattr__, 'CUI', 'Foo')


drugsData = """C0000002|ENG|P|L0000002|PF|S0000002|N|A00000002|234567|7528||RXNORM|IN|7258|Naproxen|0|N|256|
...
class TestDrug(unittest.TestCase):
    ...
    def setUp(self):
        self.drugObjects = self.make_drugObjects(StringIO(drugsData))

    def test_constructor(self):
        """Test that all drug objects got built."""
        for drugObject in self.drugObjects:
            self.assertTrue(isinstance(drugObject, rxnorm.Drug))
    ...
    def make_drugObjects(self, drugsFile):
        """Helper function that initializes a Drug object from each line in 
        the file object drugsFile."""
        drugObjects = []
    ...
    def test_semtypes_get(self):
        ...
        self.semtypes_set(self.drugObjects, StringIO(semtypesData))
        i = 0
        while i < len(self.drugObjects):
...
relsData = """C0000002|A0000002|SCUI|RN|C0000006|A0000006|SCUI|tradename_of|R49812212||RXNORM|RXNORM|||N||
...
"""
    ...
    def setUp(self):
        drugObjects = TestDrug.make_drugObjects(StringIO(drugsData)) + TestDrug.make_drugObjects(StringIO(self.drugsPenumbraData))
        TestDrug.semtypes_set(drugObjects, StringIO(semtypesData + self.semtypesPenumbra))
        concepts = {}
        for drugObject in drugObjects:
            concepts[drugObject.CUI] = drugObject
        self.relObjects = self.make_relObjects(StringIO(relsData), concepts)

    @classmethod
    def make_relObjects(self, relsFile, concepts):
        """Helper function to build a list of Relation objects from the
        given file and dictionary of CUI -> Drug mappings."""
        relObjects = []
    ...
    def test_relObjects(self):
        """Test that all Relation objects got built."""
        for relObject in self.relObjects:
            self.assertTrue(isinstance(relObject, rxnorm.Relation))

    def test_relations(self):
        """Test that the Relation objects contain the expected relations."""
        actualRels = set()
        for relObject in self.relObjects:
    ...
    def test_concept1_readonly(self):
        """Test that the .concept1 property is immutable."""
    ...
    def test_relation_readonly(self):
        """Test that the .relation property is immutable."""
        relObj = random.sample(self.relObjects, 1)
